"""
FamilyConnect - Unit tests for community_service and event_service
"""
import pytest
from uuid import uuid4
from unittest.mock import AsyncMock, patch, MagicMock

from sqlalchemy.exc import IntegrityError

from app.infrastructure.models.community import Post, Comment, PostReaction
from app.services.community_service import (
    CommunityService, NotFoundError, ForbiddenError,
    ValidationError, ConflictError,
)
from app.infrastructure.repositories.post_repository import PostRepository
from app.infrastructure.repositories.comment_repository import CommentRepository
from app.infrastructure.repositories.reaction_repository import ReactionRepository


def make_service():
    post_repo = AsyncMock()
    comment_repo = AsyncMock()
    reaction_repo = AsyncMock()
    service = CommunityService(post_repo, comment_repo, reaction_repo)
    return service, post_repo, comment_repo, reaction_repo


class TestCommunityService:

    # ============ FR-COM-01 ============

    @pytest.mark.asyncio
    async def test_create_post_success(self):
        """TC-COM-001P: Create post successfully"""
        service, post_repo, _, _ = make_service()
        post_id = uuid4()
        post_repo.create.return_value = Post(id=post_id)
        result = await service.create_post(
            family_id=uuid4(), author_id=uuid4(),
            content="  Chúc mừng!  "
        )
        assert result.id == post_id
        post_repo.create.assert_called_once()

    @pytest.mark.asyncio
    async def test_create_post_empty(self):
        """TC-COM-001N: Create post with empty content -> ValidationError"""
        service, _, _, _ = make_service()
        with pytest.raises(ValidationError):
            await service.create_post(
                family_id=uuid4(), author_id=uuid4(), content="   "
            )

    @pytest.mark.asyncio
    async def test_get_feed_success(self):
        """TC-COM-003P: Get feed"""
        service, post_repo, _, _ = make_service()
        post_repo.get_feed.return_value = []
        result = await service.get_feed(uuid4(), page=1, page_size=20)
        assert result == []

    @pytest.mark.asyncio
    async def test_get_feed_invalid_page(self):
        """TC-COM-001N: Invalid page -> ValidationError"""
        service, _, _, _ = make_service()
        with pytest.raises(ValidationError):
            await service.get_feed(uuid4(), page=0)

    @pytest.mark.asyncio
    async def test_get_feed_invalid_page_size(self):
        """TC-COM-001N: Page size > 100 -> ValidationError"""
        service, _, _, _ = make_service()
        with pytest.raises(ValidationError):
            await service.get_feed(uuid4(), page=1, page_size=101)

    @pytest.mark.asyncio
    async def test_get_post_not_found(self):
        """TC-COM-001N: Get non-existent post"""
        service, post_repo, _, _ = make_service()
        post_repo.get_by_id.return_value = None
        with pytest.raises(NotFoundError):
            await service.get_post(uuid4())

    @pytest.mark.asyncio
    async def test_update_post(self):
        """TC-COM-001P3: Author updates own post"""
        service, post_repo, _, _ = make_service()
        author = uuid4()
        post = Post(id=uuid4(), author_id=author, content="Old")
        post_repo.get_by_id.return_value = post
        post_repo.update.return_value = post
        result = await service.update_post(post.id, author, content="New")
        assert result is not None

    @pytest.mark.asyncio
    async def test_update_post_forbidden(self):
        """TC-COM-001N2: Non-author cannot update post"""
        service, post_repo, _, _ = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Old")
        post_repo.get_by_id.return_value = post
        with pytest.raises(ForbiddenError):
            await service.update_post(post.id, uuid4(), content="Hack")

    @pytest.mark.asyncio
    async def test_delete_post(self):
        """TC-COM-001P: Author deletes own post"""
        service, post_repo, _, _ = make_service()
        author = uuid4()
        post = Post(id=uuid4(), author_id=author, content="Old")
        post_repo.get_by_id.return_value = post
        await service.delete_post(post.id, author)
        post_repo.delete.assert_called_once()

    @pytest.mark.asyncio
    async def test_delete_post_forbidden(self):
        """TC-COM-001N2: Non-author cannot delete post"""
        service, post_repo, _, _ = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Old")
        post_repo.get_by_id.return_value = post
        with pytest.raises(ForbiddenError):
            await service.delete_post(post.id, uuid4())

    # ============ FR-COM-02 ============

    @pytest.mark.asyncio
    async def test_add_comment_success(self):
        """TC-COM-002P: Add comment"""
        service, post_repo, comment_repo, _ = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        comment_repo.create.return_value = Comment(id=uuid4())
        result = await service.add_comment(post.id, uuid4(), "  Chúc mừng  ")
        assert result.id is not None

    @pytest.mark.asyncio
    async def test_add_comment_empty(self):
        """TC-COM-002N: Empty comment -> ValidationError"""
        service, post_repo, _, _ = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        with pytest.raises(ValidationError):
            await service.add_comment(post.id, uuid4(), "   ")

    @pytest.mark.asyncio
    async def test_add_comment_too_long(self):
        """TC-COM-002N: Comment > 1000 chars"""
        service, post_repo, _, _ = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        with pytest.raises(ValidationError):
            await service.add_comment(post.id, uuid4(), "x" * 1001)

    @pytest.mark.asyncio
    async def test_add_reaction_success(self):
        """TC-COM-002P2: Add reaction"""
        service, post_repo, _, reaction_repo = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        reaction_repo.get.return_value = None
        reaction_repo.create.return_value = PostReaction(id=uuid4())
        result = await service.add_reaction(post.id, uuid4(), "LIKE")
        assert result.id is not None

    @pytest.mark.asyncio
    async def test_add_reaction_invalid_type(self):
        """TC-COM-002N: Invalid reaction type"""
        service, post_repo, _, _ = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        with pytest.raises(ValidationError):
            await service.add_reaction(post.id, uuid4(), "INVALID")

    @pytest.mark.asyncio
    async def test_add_reaction_duplicate(self):
        """TC-COM-002V: Duplicate reaction -> ConflictError"""
        service, post_repo, _, reaction_repo = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        reaction_repo.get.return_value = PostReaction(id=uuid4())
        with pytest.raises(ConflictError):
            await service.add_reaction(post.id, uuid4(), "LIKE")

    @pytest.mark.asyncio
    async def test_add_reaction_integrity_error(self):
        """TC-COM-002E: IntegrityError -> ConflictError"""
        service, post_repo, _, reaction_repo = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        reaction_repo.get.return_value = None
        reaction_repo.create.side_effect = IntegrityError("stmt", {}, Exception())
        with pytest.raises(ConflictError):
            await service.add_reaction(post.id, uuid4(), "LIKE")

    @pytest.mark.asyncio
    async def test_remove_reaction_success(self):
        """TC-COM-002P2: Remove reaction"""
        service, post_repo, _, reaction_repo = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        reaction_repo.delete.return_value = True
        await service.remove_reaction(post.id, uuid4(), "LIKE")

    @pytest.mark.asyncio
    async def test_remove_reaction_invalid_type(self):
        """TC-COM-002N: Invalid type"""
        service, post_repo, _, _ = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        with pytest.raises(ValidationError):
            await service.remove_reaction(post.id, uuid4(), "BAD")

    @pytest.mark.asyncio
    async def test_remove_reaction_not_found(self):
        """TC-COM-002N: Reaction not found"""
        service, post_repo, _, reaction_repo = make_service()
        post = Post(id=uuid4(), author_id=uuid4(), content="Post")
        post_repo.get_by_id.return_value = post
        reaction_repo.delete.return_value = False
        with pytest.raises(NotFoundError):
            await service.remove_reaction(post.id, uuid4(), "LIKE")

    # ============ FR-COM-05 ============

    @pytest.mark.asyncio
    async def test_create_announcement(self):
        """TC-COM-005P: Create announcement"""
        service, post_repo, _, _ = make_service()
        announcement_id = uuid4()
        post_repo.create.return_value = Post(
            id=announcement_id, visibility_scope="FAMILY"
        )
        result = await service.create_announcement(
            family_id=uuid4(), author_id=uuid4(), content="Thông báo"
        )
        assert result.id == announcement_id